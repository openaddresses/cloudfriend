...
/**
 * [The intrinsic function Fn::ForEach](https://docs.aws.amazon.com/AWSCloudFormation/latest/UserGuide/intrinsic-function-reference-foreach.html)
 * takes a collection and a fragment, and applies the items in the collection to the identifier in the provided fragment.
 *
 * @static
 * @memberof cloudfriend
 * @name forEach
 * @param {string} uniqueLoopName - The unique name of the resource loop, must be unique across all loops and resources in the template.
 * @param {string} identifier - The identifier to replace with each item in the collection.
 * @param {array} collection - The collection of items to iterate over.
 * @param {string} outputKeyPrefix - A string to prepend to the identifier of each item in the collection as it is inserted into the outputValue.
 * @param {object} outputValue - The resource fragment to insert each item in the collection into.
 * @returns {object} The outputValue with the identifier replaced with each item in the collection.
 */
intrinsic.forEach = (uniqueLoopName, identifier, collection, outputKeyPrefix, outputValue) => {
    return {
        ['Fn::ForEach::' + uniqueLoopName]: [
            identifier,
            collection,
            `${outputKeyPrefix}` + '${' + identifier + '}',
            outputValue
        ]
    };
}
